...
import { Container, Title, Group, Table, Button, Modal, Select, LoadingOverlay, NumberInput, ActionIcon } from '@mantine/core';
...
export function CourseDetails() {
  ...
  const { courses: { data: courses, isLoading: coursesLoading } } = useCourses();
  ...
  const updateGrade = useMutation({
    mutationFn: async ({ gradeId, score }: { gradeId: number; score: number | null }) => {
  ...
  return (
    <Container size="lg" pos="relative">
      ...
      <Group justify="space-between" mb="xl">
        <Title order={2}>{course?.name} ({course?.code})</Title>
        <Group>
      ...
      <Table mb="xl">
        <Table.Tbody>
          ...
          <Table.Tr>
            ...
            <Table.Td>{grades.length} / {course?.capacity}</Table.Td>
          </Table.Tr>
          <Table.Tr>
      ...
      <Title order={3} mb="md">Schedule</Title>
      <Table mb="xl">
        <Table.Thead>
          <Table.Tr>
        ...
        <Table.Tbody>
          {grades.map((grade: Grade) => (
            <Table.Tr key={grade.id}>
              ...
              <Table.Td>{grade.student?.first_name} {grade.student?.last_name}</Table.Td>
              ...
              <Table.Td>
                <Button 
                  ...
                  color="red" 
                  size="xs"
                  onClick={() => unenrollStudent.mutate(grade.id)}
                  loading={unenrollStudent.isPending}
      ...
      >
        ...
        <Group justify="flex-end">
          <Button variant="light" onClick={() => setEnrollModalOpen(false)}>Cancel</Button>
          <Button 
